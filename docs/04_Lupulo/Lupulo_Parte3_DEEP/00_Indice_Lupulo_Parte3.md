# 00 — Índice (DEEP) Lúpulo Parte 3
**Técnicas de adición**: hervor → late hopping → whirlpool/hopstand → dry hop → saturación.

## Entradas rápidas (orden recomendado)
1) [10 — Hervor y amargor](10_Hervor_y_Amargor.md)  
2) [20 — Late hopping](20_Late_Hopping.md)  
3) [30 — Whirlpool y hopstand](30_Whirlpool_y_Hopstand.md)  
4) [40 — Dry hopping](40_Dry_Hopping.md)  
5) [50 — Extracción vs saturación](50_Extraccion_vs_Saturacion.md)  
6) [60 — Puentes Obsidian (Lúpulo Parte 3)](60_Puentes_Obsidian_Lupulo_Parte3.md)

## Reglas canónicas (Parte 3)
- [R-0406 — Whirlpool/hopstand impacta amargor (no solo aroma)](30_Whirlpool_y_Hopstand.md#r-0406)
- [R-0407 — Definir duración objetivo; evitar tiempos largos por inercia](30_Whirlpool_y_Hopstand.md#r-0407)


## Objetivo
Que puedas **diseñar y controlar** el aporte del lúpulo por técnica, sin confundir:
- “más lúpulo” con “más aroma”
- “más carga” con “más calidad”
- “más tiempo” con “más extracción útil”

## Output operable (qué deberías poder decidir)
- Qué técnica conviene según objetivo: amargor / aroma / estabilidad / costo
- Dónde están los **puntos de saturación** (cuándo agregar más ya no suma)
- Cómo reducir riesgos típicos (vegetal, astringencia, oxidación, turbidez fuera de objetivo)

## Notas
- Las reglas canónicas específicas de IPA moderna (biotransformación/tioles/hop creep) están en **Parte 4**.
